...
sample_data(ps16S)$Group <- ifelse((sample_data(ps16S)$Shade_species=="None"&sample_data(ps16S)$Group=="Diversification"),"Diversification Control", paste(sample_data(ps16S)$Group)) # set up factor for diversification control (diversification plot, no adjacent shade trees)
sample_data(ps16S)$Group <- factor(sample_data(ps16S)$Group,levels=c("Diversification","Monoculture","Diversification Control"))

sample_data(psITS)$Group <- ifelse((sample_data(psITS)$Shade_species=="None"&sample_data(psITS)$Group=="Diversification"),"Diversification Control", paste(sample_data(psITS)$Group)) 
...
head(datF)
fullsoil <- left_join(datF[,c(7,10:20)],soil,by="Local.Label.y")%>% filter(complete.cases(.$Sand))%>%column_to_rownames("Local.Label.y") %>% select(-P_Olsen)
# control plot didn't have P_Olsen measured so need to remove that to keep diversification and monoculture in PCA


library(factoextra)

fullsoil2 <- fullsoil[complete.cases(fullsoil[,12:33]),]%>%select(-P2O5,-K2O,-CaO,-MgO,-pH_KCl)%>%rename(P=P_CitAcExt,pH=pH_H2O,NH4=NH4_N_mg,NO3=NO3_N_mg,Base_saturation=Base_sat,"% C"=C_percent,"% N"=N_percent,"C/N"=C_N,Cations=Total_Cations)
...
ggexport(pcaplot,filename="PCA.tiff",height=1200,width=1600,res=300)

```


Visualize properties that differ between plots
```{r}
library(ggpubr)
...
The above plots show that NH4-N, P, and pH differ by shade tree presence rather than plot. Are there effects of shade tree species on those parameters?
```{r}
...
(divITS <- ggplot(div,aes(x=Group,y=value))+
  ...
  theme(panel.grid=element_blank())+
  theme(text=element_text(size=14))+
    stat_compare_means(comparisons=list(c("Monoculture","Diversification"),c("Diversification","Diversification Control")),label="p.signif",vjust=0.2)+
  theme(axis.text.x=element_blank(),axis.title.x=element_blank(),axis.ticks.x=element_blank())+
...
(div16S <- ggplot(divB,aes(x=Group,y=value))+
  ...
  theme(panel.grid=element_blank())+
  theme(text=element_text(size=14))+
  theme(axis.text.x=element_blank(),axis.title.x=element_blank(),axis.ticks.x=element_blank())+stat_compare_means(comparisons=list(c("Monoculture","Diversification"),c("Diversification","Diversification Control")),label="p.signif",vjust=0.2)+theme(axis.title.y=element_blank())+theme(legend.position="bottom")+theme(legend.title=element_blank()))

(divplot <- ggarrange(divITS,div16S,ncol=1,common.legend=TRUE,legend="bottom"))
```
...
PCoA ordination of microbial community composition (beta diversity)
```{r}
...
(comp16S <- plot_ordination(ps16Slog, out.pcoa.logB, color = "Group") +
  ...
  theme(text=element_text(size=12),axis.title=element_text(size=12),legend.title=element_blank(),plot.title=element_text(hjust=0.5,size=12))+
  scale_color_brewer(palette="Set2")+
  theme(panel.grid=element_blank())+theme(legend.position="bottom")+ggtitle("Bacteria"))
...
(compITS <- plot_ordination(psITSlog, out.pcoa.logF, color = "Group") +
  ...
  theme(text=element_text(size=12),axis.title=element_text(size=12),legend.title=element_blank(),plot.title=element_text(hjust=0.5,size=12))+
  scale_color_brewer(palette="Set2")+
  theme(panel.grid=element_blank())+theme(legend.position="bottom")+ggtitle("Fungi"))
...
(shade16S <- plot_ordination(ps16Slog_samples, out.pcoa.logB_samples, color = "Shade_species") +
 theme_bw()+
  ...
  theme(plot.title=element_text(size=12,hjust=0.5),axis.title=element_text(size=12))+
  scale_color_brewer(palette="BrBG")+
  theme(panel.grid=element_blank())+ggtitle("Bacteria")+theme(legend.position = "bottom")+guides(col=guide_legend(ncol=3))+theme(legend.title=element_blank()))
...
(shadeITS <- plot_ordination(psITSlog_samples, out.pcoa.logF_samples, color = "Shade_species") +
  ...
  theme(text=element_text(size=12),axis.title=element_text(size=12),plot.title=element_text(size=12,hjust=0.5))+
  scale_color_brewer(palette="BrBG")+
  theme(panel.grid=element_blank())+ggtitle("Fungi")+theme(legend.position = "bottom")+theme(legend.title=element_blank())+guides(col=guide_legend(ncol=3)))
...
ggarrange(ggarrange(comp16S,compITS,ncol=2,common.legend = T,legend="bottom"),ggarrange(shade16S,shadeITS,ncol=2,common.legend=T,legend="bottom"),ncol=1,labels="auto")
```

## Distance-decay relationships
To assess distance-decay relationships (i.e. does dissimilarity between microbial communities increase with increasing geographic distance), we ran Multiple Matrix Regression (Wang et al. 2013). To visualize, we plotted Bray-Curtis distance vs. geographic distance and fit trendlines. Because we were interested in how the presence of shade trees affects these relationships, we plotted the three sample groups separately and fit separate trendlines.

First, calculate distance matrices for geographic distance
```{r }
...
soilyield2 <- soilyield %>% select(-NH4_N_mg,-C_percent,-P_CitAcExt,-CEC,-K)%>%rename("%C"="C","%N"="N_percent","NO3-N"="NO3_N_mg","pH"="pH_H2O","NH4-N"="NH4","K"="K_log","CEC"="CEC_log","Base saturation"="Base_sat")
...
mat <- t(importance2)
library(gplots)
tiff(filename='plot_yield.tiff', width=180, height=100,units="mm",res=300)
heatmap.2(x=mat,
          dendrogram="none",
          Rowv = NULL,
          Colv=NULL,
...
soilmic2 <- soilmic2 %>% select(-K,-NH4_N_mg,-C_percent,-P_CitAcExt)%>%
  rename("K"="K_log","Silt"="Loam")%>%select(Harvested_healthy,Harvested_infected,Dry_beans_kg,PC1B,PC2B,ShannonB,PielouB,RichnessB,PC1F,PC2F,ShannonF,PielouF,RichnessF,everything())
```
...
library(gplots)
tiff(filename='plot_mic_heatmap.tiff', width=180, height=180,units="mm",res=300)
heatmap.2(x=mat,
          dendrogram="none",
          Rowv = NULL,
          Colv=NULL,
...
```{r}
soilmic3 <- soilmic2[complete.cases(soilmic2$Dry_beans_kg),]%>%select(-PielouB,-PielouF,-Sand) # remove collinear variables
# missing yield data means we have only have 37 samples to work with here
...
```{r}
cor(soilmic2[1], soilmic2[c(4:27)],method="kendall",use="complete.obs")
cor.test(soilmic2$ShannonF,soilmic2$Harvested_healthy,method="kendall",use="complete.obs") # p= 0.0086 (others NS)

cor(soilmic2$Dry_beans_kg, soilmic2[c(4:27)],method="kendall",use="complete.obs")
cor.test(soilmic2$ShannonF,soilmic2$Dry_beans_kg,method="kendall",use="complete.obs") # p= 0.0049 (others NS)

cor(soilmic2$Harvested_infected, soilmic2[c(4:27)],method="kendall",use="complete.obs")
cor.test(soilmic2$NH4,soilmic2$Harvested_infected,method="kendall")
#p=0.057

# Correlation plots for significant parameters
library(grid)
library(gridExtra)
grob1 = grobTree(textGrob(paste("Kendall Correlation : ", round(cor(soilmic2$Harvested_healthy, soilmic2$ShannonF,method="kendall",use="complete.obs"), 3),"p=0.0086" ), x = 0.4, y = 0.97, hjust = 0, gp = gpar(col = "red", fontsize = 11, fontface = "bold")))

ggplot(soilmic2, aes(x=ShannonF, y=Harvested_healthy)) + geom_point() + ggtitle("Healthy pods vs. fungal diversity") + geom_smooth(method=lm, se=FALSE) + scale_x_continuous(name = "Shannon index") + scale_y_continuous(name = "Harvested pods") + theme(plot.title = element_text(hjust = 0.5), panel.background = element_blank(), axis.line = element_line(color="black"), axis.line.x = element_line(color="black"))+annotation_custom(grob1)
```
