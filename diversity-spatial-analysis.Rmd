...
```{r}
# Healthy pods
cor(soilyield2[4:18], soilyield[2],method="kendall",use="complete.obs")
...
datB2 <- data.frame(sample_data(ps16S)) %>%rownames_to_column("Rownames")%>%select(-Local.Label.y)
datF2 <- data.frame(sample_data(psITS))%>%rownames_to_column("Rownames")%>%select(-Local.Label.y)

PCBs <- data.frame(sample_data(ps16S))%>% select(Local.Label.y,PC1B,PC2B)%>%rename("Rownames"="Local.Label.y")
PCFs <- data.frame(sample_data(psITS))%>%select(Local.Label.y,PC1F,PC2F)%>%rename("Rownames"="Local.Label.y")
...
# again, transform skewed variables
...
soilmic2 <- soilmic2 %>% select(-K,-`NH4-N`,-C_percent,-P_CitAcExt)%>%select(PC1B,PC2B,PC1F,PC2F,Harvested_healthy,Harvested_infected,Dry_beans_kg,everything())
```
...
colnames(soilmic2)
library(caret)
step.PC1B <- train(PC1B ~., data = soilmic2[,c(1,11:24)],
                    method = "leapSeq", #stepwise
...
# PC2B
step.PC2B <- train(PC2B ~., data = soilmic2[,c(2,11:24)],
                    method = "leapSeq", #stepwise
                    tuneGrid = data.frame(nvmax = 1:13),
                    trControl = train.control)
...
# PC1F
step.PC1F <- train(PC1F ~., data = soilmic2[,c(3,11:24)],
                    method = "leapSeq", #stepwise
                    tuneGrid = data.frame(nvmax = 1:13),
                    trControl = train.control)
...
# PC2F
step.PC2F <- train(PC2F ~., data = soilmic2[,c(4,11:24)],
                    method = "leapSeq", #stepwise
                    tuneGrid = data.frame(nvmax = 1:13),
                    trControl = train.control)
...
# Richness
step.Richness <- train(Richness ~., data = soilmic2[,c(8,11:24)],
                    method = "leapSeq", #stepwise
                    tuneGrid = data.frame(nvmax = 1:13),
                    trControl = train.control)

plot(varImp(step.Richness)) # P, N_percent, pH
importance.mic[,5] <- varImp(step.Richness)$importance %>%rename(Richness=Overall)

# Shannon
step.Shannon <- train(Shannon ~., data = soilmic2[,c(9,11:24)],
                    method = "leapSeq", #stepwise
                    tuneGrid = data.frame(nvmax = 1:13),
                    trControl = train.control)

plot(varImp(step.Shannon)) # N_percent, NH4, Silt
importance.mic[,6] <- varImp(step.Shannon)$importance %>%rename(Shannon=Overall)
