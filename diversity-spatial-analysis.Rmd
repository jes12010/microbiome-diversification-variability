...
fullsoil <- left_join(datF[,c(7,10:24)],soil,by="Local.Label.y")%>% filter(complete.cases(.$Sand))%>%column_to_rownames("Local.Label.y") %>% select(-P_Olsen)
...
# What soil texture is this?
```{r}
fullsoil3 %>% group_by(Group) %>% summarize(Sand=mean(Sand),Silt=mean(Loam),Clay=mean(Clay))
```
...
# Dry beans
step.drybeans <- train(Dry_beans_kg ~., data = soilyield[,c(3,4:18)],
...
plot(varImp(step.drybeans)) #  P_CitAcExt,No3,Sand,Na,K
...
```{r}
step.healthy <- train(Harvested_healthy ~., data = soilyield[,c(1,4:18)],
                    method = "leapSeq", #stepwise
                    tuneGrid = data.frame(nvmax = 1:14),
                    trControl = train.control
                    )
...
```{r}
step.infected <- train(Harvested_infected ~., data = soilyield[,c(2,4:18)],
                    method = "leapSeq", #stepwise
                    tuneGrid = data.frame(nvmax = 1:14),
                    trControl = train.control
                    )
...
plot(varImp(step.infected)) # P_CitAcExt, Loam, Mg
...
```{r}
hist(soilyield$Harvested_healthy)
hist(soilyield$Harvested_infected)
hist(soilyield$Dry_beans_kg)
hist(soilyield$P_CitAcExt,breaks=c(0,10,20,40,100,1000))
hist(soilyield$NO3_N_mg)
```
...
```{r}
# Healthy pods
cor(soilyield[4:18], soilyield[3],method="kendall",use="complete.obs")
# highest absolute values for dry beans: Sand, N_percent,Loam,K,Na, none significant
# infected pods: NH4N p=0.049
# healthy pods: N_percent, but not significant
cor.test(soilyield$Harvested_healthy,soilyield$N_percent,method="kendall",use="complete.obs")


# Correlation plots for significant parameters
library(grid)
library(gridExtra)
grob1 = grobTree(textGrob(paste("Kendall Correlation : ", round(cor(soilyield$Harvested_infected, soilyield$NH4_N_mg,method="kendall"), 4) ), x = 0.57, y = 0.97, hjust = 0, gp = gpar(col = "red", fontsize = 11, fontface = "bold")))
...
# rarefy
set.seed(100)
ps16S_rare <-rarefy_even_depth(ps16S, sample.size = min(sample_sums(ps16S)))
psITS_rare <-rarefy_even_depth(psITS, sample.size = min(sample_sums(psITS)))


(bactrda <- rda(otu_table(ps16S_rare)))
summary(eigenvals(bactrda)) # only 19% of variation accounted for by first two PCs - need 16 PCs to get to 50% of variation

sample_data(ps16S_rare)$PC1B <- scores(bactrda)$sites[,1]
sample_data(ps16S_rare)$PC2B <- scores(bactrda)$sites[,2]

funrda <- rda(otu_table(psITS_rare))
sample_data(psITS_rare)$PC1F <- scores(funrda)$sites[,1]
sample_data(psITS_rare)$PC2F <- scores(funrda)$sites[,2]

# merge sample data to use for stepwise regression
datB2 <- data.frame(sample_data(ps16S_rare)) %>%rownames_to_column("Rownames")%>%select(-Local.Label.y)
datF2 <- data.frame(sample_data(psITS_rare))%>%rownames_to_column("Rownames")%>%select(-Local.Label.y)

# stuck here
PCBs <- data.frame(sample_data(ps16S_rare))%>% select(Local.Label.y,PC1B,PC2B)%>%rename("Rownames"="Local.Label.y")
PCFs <- data.frame(sample_data(psITS_rare))%>%select(Local.Label.y,PC1F,PC2F)%>%rename("Rownames"="Local.Label.y")
micPCs <- left_join(PCBs,PCFs)

fullsoil2 <- fullsoil %>% rownames_to_column("Rownames")
soilmic <- merge(fullsoil2,micPCs,by="Rownames")%>%
  select(-CaO,-MgO,-K2O,-P2O5,-pH_KCl,-Total_Cations,-C_N,-Local.Label,-Longitude,-Latitude,-Dist_shade,-Nearest_shade,-Shade_species,-Shade_Longitude,-Shade_Latitude,-Group,-Base_sat) %>%
  rename("% C" = "C_percent","% N" = "N_percent", 
         "NO3-N"="NO3_N_mg","NH4-N"="NH4_N_mg", "P"="P_CitAcExt",
         "pH"="pH_H2O",
         "Silt"="Loam")  %>% column_to_rownames("Rownames")
#7-20 have soil properties
```
...
```{r}
library(caret)
step.PC1B <- train(PC1B ~., data = soilmic[,c(7:20,21)],
                    method = "leapSeq", #stepwise
                    tuneGrid = data.frame(nvmax = 1:13),
                    trControl = train.control)
 
#summary(step.PC1B$finalModel) # ph_H2O only

plot(varImp(step.PC1B)) # pH, Mg, %N, %C 
importance.mic <- varImp(step.PC1B)$importance %>%rename(PC1B=Overall)


# PC2B
step.PC2B <- train(PC2B ~., data = soilmic[,c(7:20,22)],
                    method = "leapSeq", #stepwise
                    tuneGrid = data.frame(nvmax = 1:13),
                    trControl = train.control)
 
summary(step.PC2B$finalModel) # CEC

plot(varImp(step.PC2B)) # Clay, Mg, NH4-N
importance.mic[,2] <- varImp(step.PC2B)$importance %>%rename(PC2B=Overall)

# PC1F
step.PC1F <- train(PC1F ~., data = soilmic[,c(7:20,23)],
                    method = "leapSeq", #stepwise
                    tuneGrid = data.frame(nvmax = 1:13),
                    trControl = train.control)
 
summary(step.PC1F$finalModel) #Silt, Mg

plot(varImp(step.PC1F)) #  Na, Mg, Silt
importance.mic[,3] <- varImp(step.PC1F)$importance %>%rename(PC1F=Overall)

# PC2F
step.PC2F <- train(PC2F ~., data = soilmic[,c(7:20,24)],
                    method = "leapSeq", #stepwise
                    tuneGrid = data.frame(nvmax = 1:13),
                    trControl = train.control)
 
summary(step.PC2F$finalModel) # pH_H2O

plot(varImp(step.PC2F)) # Clay, Ca, pH
importance.mic[,4] <- varImp(step.PC2F)$importance %>%rename(PC2F=Overall)

mat <- t(importance.mic)
library(gplots)
#tiff(filename='Fig4.tiff', width=180, height=180,units="mm",res=300)
heatmap.2(x=mat,
          dendrogram="none",
          Rowv = NULL,
          Colv=NULL,
...
```{r}
hist(soilmic$P2O5,data=soilmic)

# conversion of P2O5 to ppm: lbs P2O5 / 2.3 = lbs P /2 = ppm
# so P2O5/4.6 = ppm

hist(soilmic$P2O5/4.6) #range 0-30 but most below 20
# see mineral nutrition of cacao
# for cacao, minimum is thought to be 24 ppm in clay soils and 12 ppm in sandy soils
# so P is likely inadequate/limiting here

hist(soilyield$Mg) # lower adequate limit thought to be 2 cmol/kg for cacao


hist(soilyield$N_percent) # interestingly, the majority of samples are below the recommendation of 0.15%
hist(soilyield$C_percent) # all below recommendation of 1.75%
```


# What microbial taxa might predict yield?
First, check whether addition of PCs alters the models for yield.
```{r}
soilmic2 <- soilmic[complete.cases(soilmic$Dry_beans_kg),]
# check PCs - note that missing data means we have only have 39 samples to work with here
step.dry2 <- train(Dry_beans_kg ~., data = soilmic2[,c(3,4:24)],
                    method = "leapSeq", #stepwise
                    tuneGrid = data.frame(nvmax = 1:20),
                    trControl = train.control)
summary(step.dry2$finalModel) # PC1F, P, NO3, Silt
importance.yield <- varImp(step.dry2)$importance %>%rename(Dry_beans_kg=Overall)

step.healthy2 <- train(Harvested_healthy ~., data = soilmic2[,c(1,4:24)],
                    method = "leapSeq", #stepwise
                    tuneGrid = data.frame(nvmax = 1:20),
                    trControl = train.control)
...
step.infected2 <- train(Harvested_infected ~., data = soilmic2[,c(2,4:24)],
                    method = "leapSeq", #stepwise
                    tuneGrid = data.frame(nvmax = 1:20),
                    trControl = train.control)
...
ps16S_filt <- subset_samples(ps16S_filt,!is.na(sample_data(ps16S_filt)$Dry_beans_kg)) # 39 samples
...
beans16S <- lm(Dry_beans_kg.y~P2O5+MgO,data=data.frame(sample_data(ps16S_filt)))
...
beansITS <- lm(Dry_beans_kg.y~P2O5+MgO,data=data.frame(sample_data(psITS_filt)))
...
alpha = 0.05
sigtab = res[which(res$padj < alpha), ] # 0
sigtab = cbind(as(sigtab, "data.frame"), as(tax_table(psITS_filt)[rownames(sigtab), ], "matrix")) #46
...
(plot_beansITS<-ggplot(dat_beansITS, aes(x=Dry_beans_kg.y, y=Abundance, col=Phylum)) + geom_point(aes(col=Phylum))+facet_wrap(Family~OTU, scales="free",ncol=4)+theme_bw()+theme(panel.grid=element_blank())+theme(legend.position="bottom")+scale_color_brewer(palette="Set2")+labs(x="Dry beans (kg)"))
```
