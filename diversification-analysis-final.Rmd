...
Comparison of alpha diversity by shade tree presence
```{r}
...
(divITS <- ggplot(div,aes(x=Group,y=value))+
  ...
  theme(text=element_text(size=10))+
    stat_compare_means(comparisons=list(c("Diverse-Cacao","Cacao-Cacao")),label="p.signif",vjust=0.5,size=3)+
  theme(axis.text.x=element_blank(),axis.title.x=element_blank(),axis.ticks.x=element_blank())+
...
(div16S <- ggplot(divB,aes(x=Group,y=value))+
  ...
  theme(panel.grid=element_blank())+
  theme(text=element_text(size=10))+
  theme(axis.text.x=element_blank(),axis.title.x=element_blank(),axis.ticks.x=element_blank())+stat_compare_means(comparisons=list(c("Diverse-Cacao","Cacao-Cacao")),label="p.signif",vjust=0.5,size=3)+theme(axis.title.y=element_blank())+theme(legend.position="bottom")+theme(legend.title=element_blank()))

(divplot <- ggarrange(div16S,divITS,ncol=1,labels=c("a","b"),common.legend=TRUE,legend="bottom",font.label=list(size=10)))
ggexport(divplot,filename="SuppFig2_div.tiff",width=1200,height=800,res=300)
max(sample_data(psITS)$Richness)
min(sample_data(psITS)$Richness)
```

PCoA ordination: Effect of shade tree presence
```{r}
