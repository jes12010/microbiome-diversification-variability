...
suppfig1 <- soilplot%>%
  ...
  stat_compare_means(comparisons=list(c("Diverse-Cacao","Cacao-Cacao")),label="p.signif",vjust=0.5)+theme(legend.position="bottom")
...
The above plots show that NH4-N, NO3-N, P, and pH differ by shade tree presence rather than plot. Are there effects of shade tree species on those parameters?
```{r}
...
(divITS <- ggplot(div,aes(x=Group,y=value))+
  ...
  theme(panel.grid=element_blank())+
  theme(text=element_text(size=14))+
    stat_compare_means(comparisons=list(c("Monoculture","Diversification"),c("Diversification","Diversification Control")),label="p.signif",vjust=0.5)+
  theme(axis.text.x=element_blank(),axis.title.x=element_blank(),axis.ticks.x=element_blank())+
