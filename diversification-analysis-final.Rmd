...
## Exploring microbial taxa and functions of importance
...
Which bacterial taxa are in PC1B and PC2B?
```{r}
# perform same PCA as previously
ps16S_filt <- filter_taxa(ps16S, function(x) sum(x > 1) > 3, TRUE)

(rda16S <- rda(otu_table(ps16S_filt),scale=FALSE)) # scaling would mean species have unit variance

biplot(rda16S,
       display = c("sites",  "species"),
       type = c("text","points"))

# top 100 loadings on PC1B
pc1b_top <- data.frame(scores(rda16S, choices = 1, display = "species", scaling = 0))%>% slice_max(order_by=PC1,n=100) %>% rownames_to_column("ASV") 

# top 100 loadings on PC2B
pc2b_top <- data.frame(scores(rda16S, choices = 2, display = "species", scaling = 0))%>% slice_max(order_by=PC2,n=100) %>% rownames_to_column("ASV") 

pc1b <- subset_taxa(ps16S_filt,taxa_names(ps16S_filt) %in% pc1b_top$ASV) 
pc2b <- subset_taxa(ps16S_filt,taxa_names(ps16S_filt) %in% pc2b_top$ASV)

head(tax_table(pc1b))
library(metacoder)
pc1b_obj <- parse_phyloseq(pc1b)
print(pc1b_obj)
set.seed(1)
#tiff('pc1b.tiff', units="in", width=5, height=4, res=300, compression = 'lzw')
pc1b_heat <- heat_tree(pc1b_obj, 
...
pc2b_obj <- parse_phyloseq(pc2b)
print(pc2b_obj)
set.seed(1)
#tiff('pc1b.tiff', units="in", width=5, height=4, res=300, compression = 'lzw')
pc2b_heat <- heat_tree(pc2b_obj, 
          node_label = taxon_names,
          node_size = n_obs,
          node_color = n_obs,
          node_color_axis_label= NULL,
...
print(obj)
set.seed(1)
tiff('pc2f.tiff', units="in", width=5, height=4, res=300, compression = 'lzw')
pc2f_heat <- heat_tree(obj, 
          node_label = taxon_names,
          node_size = n_obs,
          node_color = n_obs,
          node_color_axis_label= NULL,
...
(figs3 <- ggarrange(pc1b_heat,pc2b_heat,pc2f_heat,labels=c("a","b","c"),ncol=1,font.label=list(size=10)))
ggexport(figs3,filename="FigS3.tiff",width=900,height=1800,res=300)
```

# Functional predictions for PC1B and PC2B
Not very informative - large percentages of 0 values.
```{r}
library(microeco)
pc1b_micro <- microtable$new(sample_table = sample_data(pc1b), otu_table = as.matrix(t(otu_table(pc1b))), tax_table = as.data.frame(tax_table(pc1b)))
pc1b_micro$tidy_dataset() 

# create trans_func object
pc1b_func <- trans_func$new(pc1b_micro)
# identify species traits
pc1b_func$cal_spe_func(prok_database = "FAPROTAX")
pc1b_fap <- pc1b_func$res_spe_func
pc1b_fap %>% summarise(across(where(is.numeric), ~ sum(.x)))
```


```{r}
pc2b_micro <- microtable$new(sample_table = sample_data(pc2b), otu_table = as.matrix(t(otu_table(pc2b))), tax_table = as.data.frame(tax_table(pc2b)))
pc2b_micro$tidy_dataset() 

# create trans_func object
pc2b_func <- trans_func$new(pc2b_micro)
# identify species traits
pc2b_func$cal_spe_func(prok_database = "FAPROTAX")
pc2b_fap <- pc2b_func$res_spe_func
pc2b_fap %>% summarise(across(where(is.numeric), ~ sum(.x)))
```

# Functional guild assignments for PC2F
```{r}
# use microeco package with existing phyloseq objects
library(microeco)
# create microtable object
pc2f_micro <- microtable$new(sample_table = sample_data(pc2f), otu_table = as.matrix(t(otu_table(pc2f))), tax_table = as.data.frame(tax_table(pc2f)))
pc2f_micro$tidy_dataset() # have to transpose asv table or get error

# create trans_func object
pc2f_func <- trans_func$new(pc2f_micro)
...
pc2f_func$cal_spe_func(fungi_database = "FUNGuild")
pc2f_funguild <- pc2f_func$res_spe_func_raw_funguild

pc2f_funguild %>% filter(confidenceRanking=="Probable") %>% group_by(trophicMode) %>% tally()

pc2f_func$cal_spe_func(fungi_database="FungalTraits")
pc2f_fungaltraits <- pc2f_func$res_spe_func_raw_FungalTraits

pc2f_fungaltraits %>%  group_by(primary_lifestyle)%>%tally()
# fungal traits appears to be more helpful here

pc2f_fungaltraits %>% rename("Primary Lifestyle"="primary_lifestyle") %>% mutate_if(is.character, str_replace_all, pattern = '_', replacement = ' ') %>%mutate_if(is.character, str_replace_na, replacement = 'unassigned')%>% count(`Primary Lifestyle` = factor(`Primary Lifestyle`)) %>% 
    mutate(pct = prop.table(n)) %>%  ggplot(aes(x=`Primary Lifestyle`,y=pct*100,fill=`Primary Lifestyle`,label = scales::percent(pct)))+scale_fill_brewer(palette="BrBG")+geom_col()+theme_bw()+theme(panel.grid=element_blank())+theme(axis.text.x=element_text(angle=90,hjust=1))+theme(legend.position="none")+labs(y="# ASVs")+geom_text(position = position_dodge(width = .9),    # move to center of bars
              vjust = -0.5,    # nudge above top of bar
              size = 3)+lims(y=c(0,40))
